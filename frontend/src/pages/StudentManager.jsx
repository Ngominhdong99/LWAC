...
import { Users, Plus, Trash2, Edit3, X, Eye, ChevronDown } from 'lucide-react';
...
const StudentManager = () => {
  ...
  const fetchStudents = async () => {
    try {
      const res = await axios.get('http://127.0.0.1:8000/coach/students');
  ...
  const fetchLibrary = async () => {
    try {
      const res = await axios.get('http://127.0.0.1:8000/coach/library');
      setLibrary(res.data);
    } catch (e) { console.error(e); }
  };
  ...
  const handleSave = async () => {
    try {
      if (editId) {
        ...
        await axios.put(`http://127.0.0.1:8000/coach/students/${editId}`, update);
      } else {
        await axios.post('http://127.0.0.1:8000/coach/students', form);
      }
      setShowModal(false);
      setEditId(null);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:8000/coach/students/${id}`);
      fetchStudents();
  ...
  const toggleAssignments = async (id) => {
    ...
    try {
      const res = await axios.get(`http://127.0.0.1:8000/coach/students/${id}/assignments`);
  ...
  const handleAssignTest = async (lessonId) => {
    try {
      await axios.post(`http://127.0.0.1:8000/coach/students/${assigningStudentId}/assignments`, { lesson_id: lessonId });
  ...
  const handleDeleteAssignment = async (assignmentId) => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:8000/coach/assignments/${assignmentId}`);
      refreshAssignments(expandedId); // refresh assignments without collapsing
  ...
  const handleViewDetails = async (assignment) => {
    try {
      ...
      const [lessonRes, resultRes] = await Promise.all([
        axios.get(`http://127.0.0.1:8000/lessons/${assignment.lesson_id}`),
        axios.get(`http://127.0.0.1:8000/coach/results/${assignment.result_id}`)
      ]);
      setViewingResult({
        assignment,
  ...
  const handleSaveWritingFeedback = async () => {
    try {
      ...
      const res = await axios.put(`http://127.0.0.1:8000/results/${viewingResult.result.id}`, {
          responses: {
              ...currentResponses,
              ...
              {viewingResult.lesson.type === 'writing' ? (
                <div className="flex flex-col md:flex-row gap-6">
                    <div className="flex-1 bg-white p-6 rounded-xl border border-slate-200">
                       <h4 className="font-bold text-slate-800 mb-2">Student's Essay</h4>
                       <p className="text-sm text-slate-500 mb-4">Highlight text below to add specific comments.</p>
                       <div 
                           className="text-slate-700 whitespace-pre-wrap leading-relaxed selection:bg-yellow-200"
                           onMouseUp={handleTextSelection}
                           onTouchEnd={handleTextSelection}
                       >
                           {viewingResult.result.responses?.user_response || typeof viewingResult.result.responses === 'string' ? viewingResult.result.responses : JSON.stringify(viewingResult.result.responses)}
                       </div>
                    </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200">
                    ...
                    {viewingResult.result.responses?.user_audio_url ? (
                        <div className="space-y-6">
                            <audio src={viewingResult.result.responses.user_audio_url} controls className="w-full" />
